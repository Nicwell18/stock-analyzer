...
# El email lo exige la SEC. En la nube se lee desde los "secrets" de Streamlit;
# en local puedes cambiar el valor por defecto de abajo directamente.
try:
    USER_AGENT = st.secrets["SEC_USER_AGENT"]
except Exception:
...
def get_yfinance(ticker):
    ...
    yfinance raspa Yahoo Finance y a veces devuelve error de rate limit
    (sobre todo en servidores compartidos como Streamlit Cloud). Reintentamos
    ...
    for intento in range(4):
        try:
            ...
            info = None
        time.sleep(2 + intento * 2)  # 2s, 4s, 6s entre reintentos
...
def get_price_history(ticker, period="1y"):
    """Histórico de precios, con reintentos ante rate limit de Yahoo."""
